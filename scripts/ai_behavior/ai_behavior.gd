...
func get_bicircular_weight(position: Vector2, center_target: Vector2, inner_circlcle_radius: float, inner_circle_weight:float, outer_circlcle_radius: float, outer_circle_weight:float) -> float:
	var distance_to_center := position.distance_to(center_target)
	if distance_to_center > outer_circlcle_radius:
		return outer_circle_weight
	elif distance_to_center < inner_circlcle_radius:
		return inner_circle_weight
	else:
		var distance_to_inner_radius := distance_to_center - inner_circlcle_radius
		var close_range_distance := outer_circlcle_radius - inner_circlcle_radius
